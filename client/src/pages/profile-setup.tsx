...
export default function ProfileSetup() {
  ...
  async function handleSaveProfile() {
    if (!firstName.trim() || !lastName.trim()) {
      toast({ title: "Missing required answers", description: "Please complete first and last name.", variant: "destructive" });
      return;
    }
  ...
  // Only redirect to /login if we are NOT saving and we didn't just mount (prevents redirect race).
  useEffect(() => {
    if (authLoading) return;
    if (!skippedInitialRedirect.current) {
  ...
  // recovery/prescription helpers
  function addPrescription() { setPrescriptionsList((s) => [...s, { id: `${Date.now()}`, name: "", dosage: "", frequency: "", start_date: "" }]); }
  function updatePrescription(id: string, changes: Partial<any>) { setPrescriptionsList((s) => s.map((p) => (p.id === id ? { ...p, ...changes } : p))); }
  function removePrescription(id: string) { setPrescriptionsList((s) => s.filter((p) => p.id !== id)); }
  ...
  // redirect unauthenticated users handled by the guarded effect above
